...
NOTE: This doesn't allow you to enter a grandfather directly for a son.  If 
don't enter it at the time,  have to enter it as a separate father-son 
relationship.  Same for deleting - can only delete father-son pairings so can 
delete a father-son and leave the father-grandfather.
"""
    ...
    if menu_choice == "0":
        ...
        if son.title() in name_register:
            generation_choice = input("Do you want to edit the father (enter 'F')"\
                                      " or the grandfather (enter 'G')?")
            if generation_choice.upper() == "F":
                # Still have to do the title conversion when looking up in the dictionary via dictionary[key]
